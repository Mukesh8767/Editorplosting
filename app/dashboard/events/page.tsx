...
export default function AuthorEventsPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        // Only load events created by this author
        const data = await getEvents({ userId: user.id });
        setEvents(data);
      } catch (err: any) {
  ...
  const refreshEvents = async () => {
    if (!session) return;
    try {
      const data = await getEvents({ userId: session.id });
      setEvents(data);
    } catch (err: any) {
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        <header className="rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-800/50 to-slate-900/50 p-8 shadow-xl">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.32em] text-emerald-400 font-bold">Author Studio</p>
              ...
              <p className="mt-2 max-w-2xl text-base leading-7 text-slate-400">
                Manage and host your green webinars, conference panels, and interactive meetups.
              </p>
            </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.2fr_1.3fr]">
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-8 shadow-xl">
            <h2 className="text-2xl font-bold text-white mb-6">
              {eventId ? "Edit Event Details" : "Schedule Event"}
            </h2>
            ...
            <form onSubmit={handleSave} className="space-y-5">
              ...
              <div className="flex gap-3 pt-4">
                ...
                >
                  {eventId ? "Save Changes" : "Schedule Event"}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-8 shadow-xl flex flex-col h-full">
            ...
            {loading ? (
              ...
              <div className="text-slate-400 py-12 text-center border border-dashed border-slate-800 rounded-xl">
                No events found. Schedule your first event using the form.
              </div>
            ) : (
