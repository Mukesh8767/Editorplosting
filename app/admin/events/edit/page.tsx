...
import { getSession, getEvents, createEvent, type Event, type EventImage, type User } from "@/lib/blog-store";
...
function EditEventForm() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect back to overview after short delay to show message
      setTimeout(() => {
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-base text-slate-400">
              Provide event details, custom cover art, speaker listings, and registration info.
            </p>
          </div>
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="rounded-xl border border-slate-800 p-4 bg-slate-950/20 space-y-4">
            ...
            <div className="flex gap-2 items-center">
              <span className="text-xs text-slate-505 font-bold">OR</span>
              <input
                type="text"
                value={coverImageUrl}
              ...
              {coverImageUrl && (
                <button
                  ...
                  className="px-3 py-2 bg-rose-955/40 hover:bg-rose-900/60 text-rose-300 rounded-lg text-xs font-bold transition border border-rose-800/40"
                >
                  Clear
                </button>
              )}
          ...
          <div className="flex gap-4 pt-4">
            <Link
              href="/admin/events"
              className="flex-1 text-center rounded-lg border border-slate-700 bg-slate-800 px-5 py-3 text-sm font-semibold text-slate-300 hover:text-white transition"
            >
              Cancel & Return
            </Link>
            <button
